...
def _panel_title(lobby) -> str:
    return f"🎭 Lobby: {lobby.name} — {lobby.slots()} — {'abierto' if lobby.is_open else 'cerrado'}"
...
class LobbyPanel(discord.ui.View):
    """
    Vista del panel del lobby. No depende de 'bot'; todas las importaciones
    de funciones (p.ej. start_game) se hacen dentro de los handlers.
    """
    def __init__(self, lobby_name: str, *, timeout: Optional[float] = None):
    ...
    async def btn_ready(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        if ch:
            await ch.send(f"✔️ <@{inter.user.id}> está listo ({sum(1 for x in lob.players.values() if x.ready)}/{len(lob.players)})")
        await inter.response.send_message("Listo ✅", ephemeral=True)
        await update_panel(None, inter.guild, lob)
    ...
    async def btn_leave(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        if not lob or lob.name != self.lobby_name:
            return await inter.response.send_message("No estás en este lobby.", ephemeral=True)
        if not manager.leave_allowed(inter.user.id):
            return await inter.response.send_message("Debés permanecer al menos 30 segundos antes de salir.", ephemeral=True)
    ...
    async def btn_toggle_open(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        await inter.response.send_message(f"Lobby ahora está **{'abierto' if lob.is_open else 'cerrado'}**.", ephemeral=True)
        await feed.update(inter.guild)
        await update_panel(None, inter.guild, lob)
    ...
    async def btn_invite(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        class InviteModal(discord.ui.Modal, title="Invitar a jugador"):
            user_field = discord.ui.TextInput(label="@Usuario (mención o ID)", placeholder="@nombre#1234 o ID", required=True)
            ...
            async def on_submit(self, interaction: discord.Interaction):
                ...
                if interaction.guild:
                    ...
                    if member is None:
                        for m in interaction.guild.members:
                            if m.name.lower() in txt.lower() or (m.display_name and m.display_name.lower() in txt.lower()):
                                member = m
                                break
                if not member:
                ...
                await update_panel(None, interaction.guild, lob)
    ...
    async def btn_start(self, inter: discord.Interaction, button: discord.ui.Button):
        ...
        if not all(p.ready for p in lob.players.values() if not p.is_bot_sim):
            return await inter.response.send_message("Todos los jugadores (no bots) deben estar **listos**.", ephemeral=True)
...
async def update_panel(_bot: commands.Bot | None, guild: discord.Guild, lobby) -> None:
    """Crea o edita el mensaje de Panel en el canal del lobby (no usa bot)."""
